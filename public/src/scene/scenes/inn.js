...
async function setupSound(scene) {
  ...
  const musicLocal = await BABYLON.CreateStreamingSoundAsync("Long Trailer", "/assets/sounds/music/Long Trailer.mp3");
  // // Wait until audio engine is ready to play sounds.
  await AUDIO_ENGINE.unlockAsync();
  musicLocal.volume = 0.15;
  scene.activeCamera.sound.music = musicLocal;
  if (firstLoad) {
    // scene.activeCamera.sound.music.play();
  }
}
