...
export async function createHallway(engine) {
    const scene = new BABYLON.Scene(engine);

    const spawnPoint = new BABYLON.Vector3(0, 40, 0);
    ...
    camera.lowerRadiusLimit = 16;  // Minimum distance to target (closest zoom)
    ...
    const modelUrls = ["characters/enemy/slime/Slime1.glb", "characters/weapons/Sword2.glb", "util/HPBar.glb", "env/interior/hallway/hallway.glb", "util/atmosphere/lightrays/lightrays.glb"];
    ...
    let groundObject = models["hallway"].getChildMeshes()[0];
...
function addDust(scene, engine) {
    ...
    particleSystem.emitter = new BABYLON.Vector3(120.000, 200, -250);
...
function setupHallway(scene, models) {

    models["hallway"].scaling.scaleInPlace(40);
    // models["hallway"].position.x = 0;
    // models["hallway"].position.y = 0;
    // models["hallway"].position.z = -800;
    models["hallway"].getChildMeshes().forEach(mesh => {
        if (mesh.material) {
...
function setupPostProcessing(scene, camera) {
    ...
    BABYLON.Tools.LoadScriptAsync("https://cdn.jsdelivr.net/npm/lil-gui@0.17.0/dist/lil-gui.umd.min.js").then(() => {
        ...
        ssaoFolder.add(params, 'ssaoRatio', 0.1, 1, 0.1)
            .name('SSAO Ratio')
            .onChange(value => {
        ...
        ssaoFolder.add(params, 'blurRatio', 0.1, 2, 0.1)
            .name('Blur Ratio')
            .onChange(value => {
                scene.postProcessRenderPipelineManager.detachCamerasFromRenderPipeline("ssao", camera);
                ssao.dispose();
        ...
        ssaoFolder.add(params, 'expensiveBlur')
...
function createTrail(scene, engine, objectToAttach, diameter, segments, offset, rotation, scale) {
    ...
    trail.alphaIndex = 0; // Set beside fire shader
    trail.isPickable = false; // Add this line to make the trail not pickable
    // trail.billboardMode = BABYLON.Mesh.BILLBOARDMODE_ALL;
    // trail.scaling.scaleInPlace(1, 1, 1);
...
`;
    const fogMaterial = new BABYLON.ShaderMaterial("fogMaterial", scene, {
        vertexSource: vertexShader,
        fragmentSource: fragmentShader,
    }, {
        attributes: ["position", "uv"],
        uniforms: [
            "world", "worldView", "worldViewProjection", "cameraPosition",
            "time", "density", "fogColor"
        ]
    });
...
fogMaterial.transparencyMode = BABYLON.Material.MATERIAL_ALPHABLEND;
    ...
    BABYLON.Tools.LoadScriptAsync("https://cdn.jsdelivr.net/npm/lil-gui@0.17.0/dist/lil-gui.umd.min.js").then(() => {
        ...
        gui.add(fogParams, "density", 0, 0.005, 0.0001).name("Density").onChange(value => {
            fogMaterial.setFloat("density", value);
        });

        gui.addColor(fogParams, "fogColor").name("Fog Color").onChange(value => {
            fogMaterial.setVector3("fogColor", new BABYLON.Color3(value[0]/255, value[1]/255, value[2]/255));
        });

        gui.add(fogParams, "boxSize", 100, 1000, 10).name("Box Size").onChange(value => {
            fogBox.scaling = new BABYLON.Vector3(value / 10, value / 10, value / 10);
        });

        gui.add(fogParams, "positionY", 0, 100, 1).name("Position Y").onChange(value => {
            fogBox.position.y = value;
        });

        gui.add(fogParams, "positionX", -500, 500, 1).name("Position X").onChange(value => {
