...
export async function createNight(engine) {
  ...
  BABYLON.NodeMaterial.ParseFromSnippetAsync("#AT7YY5#132", scene).then(
  ...
  var iChannel0 = new BABYLON.Texture(
    "assets/textures/effects/ripple.png",
    scene
  );
  ...
  if (
    specificChild.material &&
    specificChild.material instanceof BABYLON.PBRMaterial
  ) {
  ...
  function findAllMeshesByName(meshes, name) {
    ...
    meshes.forEach((mesh) => {
      ...
      if (mesh.getChildren) {
        foundMeshes = foundMeshes.concat(
          findAllMeshesByName(mesh.getChildren(), name)
        );
      }
    });
    return foundMeshes;
  ...
  // Usage
  const allMeshesWithName = findAllMeshesByName(
    scene.meshes,
    "mixamorig:RightHand"
  );
  console.log(allMeshesWithName[0]);
  allMeshesWithName[0].isPickable = false;
  ...
  let BridgeCollision = new BABYLON.PhysicsAggregate(
    Bridge,
...
function createFogSwirl(scene, engine) {
  ...
  particleSystem.particleTexture = new BABYLON.Texture(
    "./assets/textures/effects/fogSwirl3.png",
    scene
  );
...
function createSpellParticles(scene, engine) {
  ...
  particleSystem.particleTexture = new BABYLON.Texture(
    "./assets/textures/effects/flare.png",
    scene
  );
...
async function createTerrainDiamond(scene, maxHeight) {
  var terrain = await BABYLON.MeshBuilder.CreateGroundFromHeightMap(
    "terrain",
    "/assets/textures/terrain/diamondHeight.png",
    {
      width: 10000,
      height: 10000,
      subdivisions: 100,
      ...
      onReady: () => {
        ...
        let groundAggregate = new BABYLON.PhysicsAggregate(
          terrain,
          BABYLON.PhysicsShapeType.MESH,
          { mass: 0, restitution: 0.0, friction: 1.0 },
          scene
...
function setupEnvironment(scene) {
  ...
  const environmentMap = BABYLON.CubeTexture.CreateFromPrefilteredData(
    environmentURL,
    scene
  );
...
function setupTerrain(scene) {
  ...
  terrainMaterial.mixTexture = new BABYLON.Texture(
  ...
  terrainMaterial.diffuseTexture1.uScale =
    terrainMaterial.diffuseTexture1.vScale = 15;
  terrainMaterial.diffuseTexture2.uScale =
    terrainMaterial.diffuseTexture2.vScale = 8;
  terrainMaterial.diffuseTexture3.uScale =
    terrainMaterial.diffuseTexture3.vScale = 23;
...
function setupLighting(scene) {
  ...
  lightDirectional.diffuse = new BABYLON.Vector3(0, 0.23, 0.3);
