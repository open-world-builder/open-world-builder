...
import { Options, setupOptions } from "/src/utils/options/options.js";
...
import { Interact } from "../../utils/interact/interact.js";

export async function createCastle(engine) {
  ...
  // camera.upperBetaLimit = 1.8; // A little below horizon, but skybox breaks a litte
  // camera.upperBetaLimit = Math.PI / 2; // Stops at the horizon (90 degrees)
  camera.alpha = 0.5077;
  camera.beta = 1.4437;
  ...
  PLAYER = character;
  setupPlayerHealthBar(scene, PLAYER);
  scene.WORLD_ID = "1";
  ...
  //load recast to hide delay during loading text
  loadHPModels(scene, engine, models["HPBar"]);
  await setupNPCs(scene, spawnPoint, camera, character); //remove await for speed, keep awat for loading smoothness
  ...
  setTimeout(async () => {
    ...
    setTimeout(() => {
      ...
      addWaterEffects(scene, dummyAggregate, hero);
    ...
    addSlidings(scene, dummyAggregate);
...
function toggleNavMesh(scene) {
  scene.onKeyboardObservable.add((kbInfo) => {
    if (kbInfo.type === BABYLON.KeyboardEventTypes.KEYDOWN && kbInfo.event.key === "l") {
      scene.getMeshByName("navmeshdebug").isVisible = !scene.getMeshByName("navmeshdebug").isVisible;
    }
  });
}

function addSlidings(scene, dummyAggregate) {
  scene.onKeyboardObservable.add((kbInfo) => {
    if (kbInfo.type === BABYLON.KeyboardEventTypes.KEYDOWN) {
      switch (kbInfo.event.key.toLowerCase()) {
        case "x":
          // Trigger slide
          dummyAggregate.startSlide();
          break;
      }
    }
  });
...
function setupDoor(scene, spawnPoint) {
  const door = BABYLON.MeshBuilder.CreateBox("door", { size: 8 }, scene);
  ...
  // Set pivot point for proper door rotation (optional, adjust as needed)
  door.setPivotPoint(new BABYLON.Vector3(-4.0, 0, 0)); // Rotate around left edge

  door.interact = new Interact();
  door.interact.addCustomAction("open", () => {
    toggleDoor(door, scene);
  });
  door.interact.setDefaultAction("open");

  // Simple door toggle function using Babylon animations
  function toggleDoor(door, scene) {
    if (door.isOpen) {
      // Close the door - rotate back to 0 degrees
      animateDoorRotation(door, 0, scene);
      door.isOpen = false;
      console.log("Door closed");
    } else {
      // Open the door - rotate to 90 degrees
      animateDoorRotation(door, Math.PI / 2, scene);
      door.isOpen = true;
      SCENE_MANAGER.switchToScene(1);
      window.TERRAIN_EDITOR.setTerrainTool("adventure");
  ...
  function animateDoorRotation(door, targetRotation, scene) {
    ...
    const keys = [
      ...
      { frame: 30, value: targetRotation }, // 1 second at 30 fps
    ];
    ...
    door.animations = [animation]; // Add this line!
...
function setupPlayerHealthBar(scene, player) {
  // player.healthBar = attachHealthBarToCamera(player);
}

function loadHPModels(scene, engine, HPBar) {
  HPBAR = HPBar;
  HPBAR.position.y += 13.2;
...
function createWaterParticles(scene) {
  // Create particle system for splash effect
  const splashParticles = new BABYLON.ParticleSystem("splashParticles", 2000, scene);

  // Particle texture
  splashParticles.particleTexture = new BABYLON.Texture("./assets/textures/effects/flare.png", scene);

  // Set particle behavior
  splashParticles.minLifeTime = 0.3;
  splashParticles.maxLifeTime = 1.5;
  splashParticles.minSize = 0.1;
  splashParticles.maxSize = 0.5;
  splashParticles.emitRate = 500;

  // Set emission properties
  splashParticles.minEmitBox = new BABYLON.Vector3(-1, 0, -1);
  splashParticles.maxEmitBox = new BABYLON.Vector3(1, 0, 1);

  // Set particle colors
  splashParticles.color1 = new BABYLON.Color4(0.7, 0.8, 1.0, 1.0);
  splashParticles.color2 = new BABYLON.Color4(0.2, 0.5, 1.0, 1.0);
  splashParticles.colorDead = new BABYLON.Color4(0, 0, 0.2, 0.0);

  // Set particle motion
  splashParticles.minEmitPower = 1;
  splashParticles.maxEmitPower = 3;
  splashParticles.gravity = new BABYLON.Vector3(0, -9.81, 0);

  // Add size gradients for better visual effect
  splashParticles.addSizeGradient(0, 0.0);
  splashParticles.addSizeGradient(0.3, 0.5);
  splashParticles.addSizeGradient(0.7, 0.5);
  splashParticles.addSizeGradient(1.0, 0.0);

  return splashParticles;
}

function createRippleParticles(scene) {
  // Create particle system for ripple effect
  const splashParticles = new BABYLON.ParticleSystem("splashParticles", 2000, scene);
  ...
  splashParticles.minLifeTime = 0.3;
  splashParticles.maxLifeTime = 1.5;
  splashParticles.minSize = 50.1;
  splashParticles.maxSize = 50.5;
  splashParticles.emitRate = 500;

  splashParticles.blendMode = BABYLON.ParticleSystem.BLENDMODE_ADD;
  // splashParticles.isBillboardBased = false;
  splashParticles.minScaleX = 80.5;
  splashParticles.maxScaleX = 80.5;
  splashParticles.minScaleY = 10.5;
  splashParticles.maxScaleY = 10.5;

  // Add rotation to make particles horizontal
  // splashParticles.minInitialRotation = -Math.PI / 2; // -90 degrees
  // splashParticles.maxInitialRotation = -Math.PI / 2; // -90 degrees

  splashParticles.direction1 = new BABYLON.Vector3(0, 0, 0);
  splashParticles.direction2 = new BABYLON.Vector3(0, 0, 0);

  // Create a rotation matrix to control emission direction
  splashParticles.particleEmitterType = new BABYLON.BoxParticleEmitter();
  splashParticles.particleEmitterType.direction1 = splashParticles.direction1;
  splashParticles.particleEmitterType.direction2 = splashParticles.direction2;
  ...
  // Set particle colors
  splashParticles.color1 = new BABYLON.Color4(0.7, 0.8, 1.0, 0.015);
  splashParticles.color2 = new BABYLON.Color4(0.2, 0.5, 1.0, 0.015);
  splashParticles.colorDead = new BABYLON.Color4(0, 0, 0.2, 0.0);

  // Set particle motion
  splashParticles.minEmitPower = 1;
  splashParticles.maxEmitPower = 3;
  splashParticles.gravity = new BABYLON.Vector3(0, 0, 0);
...
function addWaterEffects(scene, dummyAggregate, hero) {
  // Create cool sword effects on items. can atatch particle systems to items
  // Add distortion shader to help the particle systems siloute from just basic shapes, no texture needed
  // let splashParticles = createWaterParticles(scene);
  let splashParticles = createRippleParticles(scene);
  let isInWater = false;

  scene.onBeforeRenderObservable.add(() => {
    splashParticles.emitter = new BABYLON.Vector3(
      dummyAggregate.body.transformNode._absolutePosition.x,
      -799.9, // Keep splash at water level
      dummyAggregate.body.transformNode._absolutePosition.z
    );
    ...
    // splashParticles.particleSystem.setEmitterRotation(
    //   new BABYLON.Vector3(
    //     0,
    //     90, // This should be your player's Y rotation
    //     0
    //   )
    // );

    // Get character's rotation
    // const characterRotation = dummyAggregate.body.transformNode.rotation;

    // Create rotation matrix from character's rotation
    // const rotationMatrix = BABYLON.Matrix.RotationYawPitchRoll(characterRotation.y, characterRotation.x, characterRotation.z);

    // Apply rotation to particle emitter
    // splashParticles.particleEmitterType.direction1 = BABYLON.Vector3.TransformNormal(new BABYLON.Vector3(0, 0, -0.1), rotationMatrix);
    // splashParticles.particleEmitterType.direction2 = BABYLON.Vector3.TransformNormal(new BABYLON.Vector3(0, 0, 0.1), rotationMatrix);

    if (dummyAggregate.body.transformNode._absolutePosition.y < -792.9) {
      if (!isInWater) {
        // Just entered water
        isInWater = true;
        splashParticles.start();
      }
      if (dummyAggregate.body.getLinearVelocity().x > 0.1 || dummyAggregate.body.getLinearVelocity().x < -0.1 || dummyAggregate.body.getLinearVelocity().z > 0.1 || dummyAggregate.body.getLinearVelocity().z < -0.1) {
        if (dummyAggregate.body.transformNode._absolutePosition.y < -792.9) {
          splashParticles.emitRate = 100;
        }
        if (dummyAggregate.body.transformNode._absolutePosition.y < -798.9) {
          splashParticles.emitRate = 500;
        }
      } else {
...
async function setupSound(scene, camera) {
  ...
  setTimeout(async function () {
    ...
    // Wait until audio engine is ready to play sounds.
    await AUDIO_ENGINE.unlockAsync();
    ambience.loop = true;
  ...
  setTimeout(async function () {
    ...
    scene.activeCamera.sound.musicInterval = setInterval(async function () {
      sound.play();
