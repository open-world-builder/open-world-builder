export function setupWater(scene, ground, engine, player, y, size) {
  ...
  shaderMaterial.setVector4("vFogInfos", new BABYLON.Vector4(scene.fogMode, scene.fogStart, scene.fogEnd, scene.fogDensity));
...
export function setupWaterNoFog(scene, ground, engine, player, y, size, background) {
  //use terrain width and height
  var water = BABYLON.MeshBuilder.CreateGround("water", { width: size, height: size }, scene);
  water.position.y = y;
  ...
  shaderMaterial.setFloat("wNoiseOffset", 0.01);
  shaderMaterial.setFloat("fNoiseScale", 0.3);
  shaderMaterial.setFloat("maxDepth", 30.0); //20
  shaderMaterial.setVector4("wDeepColor", new BABYLON.Vector4(0.1, 0.2, 0.3, 0.3));
  shaderMaterial.setVector4("wShallowColor", new BABYLON.Vector4(0.1, 0.2, 0.3, 0.0)); //0.4 //0.1
  ...
  var waterUnderside = BABYLON.MeshBuilder.CreateGround("waterUnderside", { width: size, height: size }, scene);
  waterUnderside.position.y = y - 0.001;
  waterUnderside.isPickable = false;
  waterUnderside.material = shaderMaterial;
  // Rotate the waterUnderside mesh 180 degrees around the X-axis
  waterUnderside.rotation.x = Math.PI; // 180 degrees in radians
  ...
  if (debug) {
    BABYLON.Tools.LoadScriptAsync("https://cdn.jsdelivr.net/npm/lil-gui@0.17.0/dist/lil-gui.umd.min.js").then(() => {
      ...
      const settings = {
        ...
        fNoiseScale: 0.3,
        maxDepth: 30.0,
        deepColor: [0.1 * 255, 0.2 * 255, 0.3 * 255],
        deepAlpha: 0.3,
        shallowColor: [0.1 * 255, 0.2 * 255, 0.3 * 255],
        shallowAlpha: 0.01,
      };

      // Noise controls
      waterFolder
        .add(settings, "wNoiseScale", 0.01, 1.0, 0.01)
        .name("Wave Noise Scale")
        .onChange((value) => shaderMaterial.setFloat("wNoiseScale", value));

      waterFolder
        .add(settings, "wNoiseOffset", 0.001, 0.1, 0.001)
        .name("Wave Noise Offset")
        .onChange((value) => shaderMaterial.setFloat("wNoiseOffset", value));

      waterFolder
        .add(settings, "fNoiseScale", 0.1, 1.0, 0.1)
        .name("Foam Noise Scale")
        .onChange((value) => shaderMaterial.setFloat("fNoiseScale", value));

      // Depth control
      waterFolder
        .add(settings, "maxDepth", 1.0, 50.0, 1.0)
        .name("Max Depth")
        .onChange((value) => shaderMaterial.setFloat("maxDepth", value));

      // Deep water controls
      const deepFolder = waterFolder.addFolder("Deep Water");
      deepFolder
        .addColor(settings, "deepColor")
        .name("Color")
        .onChange((value) => {
          shaderMaterial.setVector4("wDeepColor", new BABYLON.Vector4(value[0] / 255, value[1] / 255, value[2] / 255, settings.deepAlpha));
        });

      deepFolder
        .add(settings, "deepAlpha", 0, 1, 0.01)
        .name("Alpha")
        .onChange((value) => {
          const color = settings.deepColor;
          shaderMaterial.setVector4("wDeepColor", new BABYLON.Vector4(color[0] / 255, color[1] / 255, color[2] / 255, value));
        });

      // Shallow water controls
      const shallowFolder = waterFolder.addFolder("Shallow Water");
      shallowFolder
        .addColor(settings, "shallowColor")
        .name("Color")
        .onChange((value) => {
          shaderMaterial.setVector4("wShallowColor", new BABYLON.Vector4(value[0] / 255, value[1] / 255, value[2] / 255, settings.shallowAlpha));
        });

      shallowFolder
        .add(settings, "shallowAlpha", 0, 1, 0.01)
        .name("Alpha")
        .onChange((value) => {
          const color = settings.shallowColor;
          shaderMaterial.setVector4("wShallowColor", new BABYLON.Vector4(color[0] / 255, color[1] / 255, color[2] / 255, value));
        });
